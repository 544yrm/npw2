...
ESX.RegisterServerCallback('vangelicoheist:server:checkTime', function(source, cb)
    ...
    if (os.time() - lastrob) < Config['VangelicoHeist']['nextRob'] and lastrob ~= 0 then
        ...
        lastrob = os.time()
        start = true
        discordLog(player.getName() ..  ' - ' .. player.getIdentifier(), ' wystartował Napad na Jubilata!')
        cb(true)
    end
end)
...
RegisterNetEvent("vangelicoheist:server:policeAlert", function(coords)
    ...
    for i=1, #xPlayers, 1 do
        ...
        if xPlayer.job.name == 'police' then
            TriggerClientEvent('vangelicoheist:client:policeAlert', xPlayers[i], _source, coords, hour)
        elseif xPlayer.job.name == 'ambulance' then
            TriggerClientEvent('vangelicoheist:client:ambulance', xPlayers[i])
        end
    end
end)
