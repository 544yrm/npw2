...
RegisterNetEvent('vangelicoheist:client:ambulance', function()
    local ped = PlayerPedId()
    SetPedRelationshipGroupHash(ped, GetHashKey('Police'))
end)

RegisterNetEvent('vangelicoheist:client:policeAlert')
AddEventHandler('vangelicoheist:client:policeAlert', function(target, coords, hour)
    local ped = PlayerPedId()
    SetPedRelationshipGroupHash(ped, GetHashKey('Police'))
	local s1, s2 = Citizen.InvokeNative(0x2EB41072B4C1E4C0, coords.x, coords.y, coords.z, Citizen.PointerValueInt(), Citizen.PointerValueInt())
	exports["amg_Powiadomienia"]:policeNotify({
		playerId = target,
		coords = coords,
		textColor = "#fff",
		bgColor = "#06a5ff",
...
function VangelicoSetup()
    ...
    TriggerServerEvent('vangelicoheist:server:policeAlert', vector3(-628.1321, -235.3328, 38.0506))
end
